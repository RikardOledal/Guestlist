...
class ProductStore:
    ...
    def add(self, product, amount):
        ...
        self.discountname.append({
            "name": product.name,
            "discount": 0
        })
        self.discounttype.append({
            "type": product.type,
            "discount": 0
        })
    ...
    def set_discount(self, identifier, percent, identifier_type="name"):
        ...
        if identifier_type == "name":
            ...
            raise ValueError("identifier_type should be name or type")
    ...
    def sell_product(self, product_name, amount):
        for p in self.products:
            ...
            for dn in self.discountname:
                name_discount = dn["name"]

                if name_discount != product_name:
                    continue
                else:
                    name_discount = dn["discount"]
            
            else:
                name_discount = (100 - self.discountname.get("Ramen"))/100

            if self.discounttype.get(product_in_store.type) is None:
                type_discount = 1
            else:
                type_discount = (100 - self.discounttype.get(product_in_store.type))/100

            print(f"Type discount:{type_discount} Name discount:{name_discount}")

            p['amount'] -= amount

            self.income += product_in_store.price * amount * name_discount * type_discount
...
def main():
    ...
    store.add(p1, 10)
    store.add(p2, 100)
    print(store.discountname)
    print(store.discounttype)
    ...
    store.set_discount("Ramen", 80, "name")
    print(store.discountname)
    print(store.discounttype)
    print("Selling 10 ramens...")
    store.sell_product("Ramen", 10)
